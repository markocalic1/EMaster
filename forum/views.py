...
@login_required()
def forum(request):
    topic_list = Topic.objects.all().order_by('-comment_count')
    ...
    if add_new_topic.is_valid():
        ...
        slug = slugify(instance.new_title)
...
def topic(request, slug):
    ...
    context = {
        "title": Topic.objects.get(slug=slug).new_title,
        "add_new_comment_form": add_new_comment,
        "path": path,
        "comment_list": comment_list,
        "first_comment": Topic.objects.get(slug=slug).message,
